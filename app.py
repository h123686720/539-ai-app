...
if st.session_state["step"] == "login":
    ...
    try:
        ...
        st.markdown(f"<div class='history-text'>📡 成功解析 {len(df)} 期歷史數據 | 權重優化已完成</div>", unsafe_allow_html=True)
        ...
        sv_display = "04, 19"
        jt_display = "07, 22, 31"
        st.markdown("<div class='history-text'>⚠️ 數據同步異常：啟動備用 AI 模型</div>", unsafe_allow_html=True)
    ...
    st.markdown("""
        <div class='stats-box'>
            ...
            <p style='font-size:11px; color:#666; margin-top:10px;'>*數據由 AI 神經網絡根據 LDC 歷史數據庫計算得出*</p>
        </div>
    """, unsafe_allow_html=True)
